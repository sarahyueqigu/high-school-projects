...
# Difficulties: Initally in my design document, I had a function called indices_list()
...
# but I quickly reqlized that my design was inefficient because it first had to
# check whether or not the user's response was valid at all. Then, it had to check
# its individual values. So, I changed it to check for the invidivdual values directly,
...
# I also originally had valid_move() determine the indices of the user's response
# as well as print the error statements if needed. Later, I realized that this
# would technically count as a function that was both calculating and printing,
# so I separated the two purposes and created a new function called invalid_move().

# Sarah Gu

# Determne whether or not the symbol given by the user is valid. Provide
...
def occupied(entireList, cellValueRow, elementIndex, currentPlayer1, opponent):
    ...
    and entireList[cellValueRow][elementIndex] != opponent:
        ...
        validity = False
        occupied2 = True
    return validity, occupied2

# Determine whether or not the user's move is valid, and call for the function
# that prints the appropriate statement if it is not. If the user's move is valid,
# call for the function that determines if the position has already been occupied.
def valid_move(userMove, playerSymbol, listOfCells, opponentSymbol):
    if userMove  == '4' or userMove == '5' or userMove  == '6':
        ...
        validAnswer, occupied3 = occupied(listOfCells, rowInCellValue, elementInRow, playerSymbol, opponentSymbol) # Check
        # to see if the cell is occupied
    elif userMove == '1' or userMove == '2' or userMove == '3':
        rowInCellValue = 0
        elementInRow = int(userMove) - 1
        validAnswer, occupied3 = occupied(listOfCells, rowInCellValue, elementInRow, playerSymbol, opponentSymbol)
    ...
    if validAnswer == False:
        invalid_move(userMove, occupied3)
    return validAnswer, occupied3

# Print the appropriatae statements if the user's response is invalid. (This function
# was created just to separate calculating functions from printing functions)
def invalid_move(userMove1, occupied1):
    if occupied1 == True:
        print('This cell is already occupied. Choose a different position.')
    try:
        userMove1 = float(userMove1)
        if userMove1 < 1:
...
while win == False and counter < 9:
    if counter % 2 == 0:
        ...
        otherPlayer = name2 # Need this to distinguish one player from the ohter in valid_move()
    elif counter % 2 == 1:
        currentPlayer = name2
        currentSymbol = symbol2
        otherPlayer = name1 # Need this to distinguish one player from the ohter in valid_move()
    position = input(currentPlayer + ', enter your choice: ')
    answerValid, occupied4 = valid_move(position, currentSymbol, cellValue, otherPlayer)
    ...
    counter += 1
    if win == True:
        print('Winner:', currentPlayer + '. Congradulations!')
